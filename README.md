...
Terraformにより24リソースを一括構築し、AWS上での動作確認を完了しています。

| コンポーネント | 内容 | ステータス |
|------------|------|----------|
| VPC / ネットワーク | 2AZ構成・パブリック/プライベートサブネット・IGW・NAT GW | ✅ 構築済み |
| EC2 | t2.micro・Amazon Linux 2023・Flask(Port 5000) | ✅ 構築済み |
| RDS | db.t3.micro・MySQL 8.0・プライベートサブネット配置 | ✅ 構築済み |
| ALB | Internet-facing・2AZ対応・EC2ターゲット | ✅ 構築済み |
| セキュリティグループ | 最小権限・ALB→EC2→RDSの連鎖設定 | ✅ 構築済み |
| IaC | Terraform（terraform plan/apply/destroy） | ✅ 運用済み |
...
### VPC リソースマップ（2AZ・サブネット・NAT GW構成）
![VPC Resource Map](docs/screenshots/01-vpc-resource-map.png)

### ALB（Application Load Balancer）
![ALB](docs/screenshots/03-alb.png)

### EC2 インスタンス（実行中）
![EC2](docs/screenshots/02-ec2.png)

### RDS（利用可能・プライベートサブネット配置）
![RDS](docs/screenshots/04-rds.png)
...
- `terraform destroy` による確実なリソース削除（立てっぱなし防止）
- AWS Budgetsによる予算アラート設定（コスト超過の早期検知）
...
│   ├── security-design.md       # セキュリティ設計書（SG・暗号化・IAM設計）
